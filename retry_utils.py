...
import functools
import random
from typing import Callable, TypeVar, Any, Dict, Optional, List, Union, Tuple
...
def is_retryable_error(error: Exception) -> bool:
    ...
    if any(network_err in str(error).lower() for network_err in 
           ['connection', 'timeout', 'reset', 'refused', 'network']):
        return True
        ...
        def wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            try:
                ...
                if state['status'] == 0 and state['failures'] >= failure_threshold:
                    # Transition to open
                    logger.warning(f"Circuit for {func.__name__} transitioning from closed to open "
