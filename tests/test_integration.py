...
class TestIntegration(unittest.TestCase):
    ...
    def test_authorization_workflow(self):
        ...
        with patch("app.exchange_code_for_tokens") as mock_exchange:
            ...
            response = self.client.post("/submit_auth_code", data={"code": "test_auth_code"})
            self.assertEqual(response.status_code, 200)
    ...
    def test_complete_folder_deletion_workflow(self, mock_delete, mock_auth, mock_check_token):
        ...
        data = {"folder_path": "integration/test/folder"}

        response = self.client.post("/delete_folder", data=data)
        self.assertEqual(response.status_code, 200)
    ...
    def test_error_handling_integration(self):
        ...
        with patch("app.check_token_exists") as mock_check_token:
            ...
            data = {"folder_path": "test/folder", "file": (io.BytesIO(self.test_file_content), "test.txt")}
    ...
    def test_content_type_handling(self):
        ...
        response = self.client.post(
            "/submit_auth_code", data=json.dumps({"code": "test"}), content_type="application/json"
        )
        # Should not crash (might return error due to mocking, but shouldn't be 500)
        self.assertNotEqual(response.status_code, 500)

        # Test multipart form data
        data = {"folder_path": "test", "file": (io.BytesIO(b"test"), "test.txt")}
        response = self.client.post("/upload_file", data=data)
        # Should not crash (might return error due to mocking, but shouldn't be 500)
        self.assertNotEqual(response.status_code, 500)
